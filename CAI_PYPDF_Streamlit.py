...
def retrieve_chunks_vector_db(query, vector_db, chunks, embedding_model_name="all-MiniLM-L6-v2", top_k=3):
    ...
    query_embedding = embed_chunks([query], embedding_model_name)[0]  # Embed the query
    D, I = vector_db.search(np.array([query_embedding]).astype('float32'), top_k)  # Search in FAISS
    retrieved_chunks = [chunks[i] for i in I[0]]  # Get chunks based on index
    return retrieved_chunks

def create_bm25_index(chunks):
    """Creates a BM25 index from the text chunks."""
    tokenized_chunks = [chunk.split() for chunk in chunks]  # Simple whitespace tokenization
...
def retrieve_chunks_bm25(query, bm25_index, chunks, reranking_model_name="cross-encoder/ms-marco-MiniLM-L-12-v2", top_k_bm25=10, top_k_rerank=3):
    ...
    top_chunk_indices_bm25 = np.argsort(bm25_scores)[::-1][:top_k_bm25]  # Get indices of top_k_bm25 BM25 chunks
    retrieved_chunks_bm25 = [chunks[i] for i in top_chunk_indices_bm25]  # Retrieve top BM25 chunks

    # Re-ranking with Cross-Encoder
    rerank_inputs = [[query, chunk] for chunk in retrieved_chunks_bm25]  # Prepare inputs for re-ranking

    cross_encoder = CrossEncoder(reranking_model_name)  # Initialize CrossEncoder model
    rerank_scores = cross_encoder.predict(rerank_inputs)  # Get re-ranking scores

    # Sort chunks based on re-ranking scores
    chunk_score_pairs = list(zip(retrieved_chunks_bm25, rerank_scores))  # Pair chunks with their scores
    sorted_chunk_score_pairs = sorted(chunk_score_pairs, key=lambda x: x[1], reverse=True)  # Sort by score
    reranked_chunks = [chunk for chunk, score in sorted_chunk_score_pairs[:top_k_rerank]]  # Get top_k_rerank chunks
...
if uploaded_file is not None:
    ...
    retrieval_method = st.radio(
        ...
        ("1. Basic RAG Search", "2.Advanced RAG Search BM25 based")
    )
